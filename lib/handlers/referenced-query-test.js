...
test('should dispatch GET action for relationship query with paging', async (t) => {
  const dispatch = sinon.stub().resolves({ status: 'ok', data: [ent1] })
  const request = {
    method: 'GET',
    type: 'entry',
    params: {
      id: 'ent1',
      page: 'eyJ0eXBlIjoiY29tbWVudCIsInBhZ2VTaXplIjoyLCJwYWdlQWZ0ZXIiOiJjb21tZW50MiJ9'
    },
    relationship: 'comments',
    path: '/entries/ent1/comments',
    headers: { authorization: `Bearer ${validJwt}` }
