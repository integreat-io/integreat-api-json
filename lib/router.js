...
const createActionFromRelationship = (relationship, method) => {
  if (Array.isArray(relationship)) {
    if (relationship.length === 0) {
      return null
    }
    const ids = relationship.map((rel) => rel.id)
    return createAction(method, relationship[0].type, ids)
  } else {
...
function router (dispatch) {
  return async (request) => {
    ...
    if (id && relationship) {
      ...
      if (result.status === 'ok') {
        action = createActionFromRelationship(result.data[0].relationships[relationship], method)
      } else {
