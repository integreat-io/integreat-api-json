...
const actionFromRelData = ({data}, method) => {
  if (Array.isArray(data)) {
    if (data.length === 0) {
      return null
    }
    const ids = data.map((rel) => rel.id)
    return createAction(method, {type: data[0].type, id: ids})
  } else {
    const {id, type} = data
  ...
  return async (request) => {
    ...
    if (result.status === 'ok' && isRelationshipResource) {
      ...
      if (relData === null) {
        ...
        if (!action) {
          // No action means there where no ids. Return with empty array
          return createResponse({status: 'ok', data: []}, request, null)
        }
      } else {
