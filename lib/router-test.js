...
test('should dispatch GET action for relationship resource', async (t) => {
  const dispatch = sinon.stub().resolves({status: 'ok', data: [entry1]})
  const request = {
    method: 'GET',
    type: 'entry',
...
test('should return 404 for relationship request', async (t) => {
...
test('should dispatch SET action for POST request', async (t) => {
  const dispatch = sinon.stub().resolves({status: 'ok', data: [entry1]})
  const request = {
    method: 'POST',
    type: 'entry',
...
test('should return response from POST request', async (t) => {
  const dispatch = async () => ({status: 'ok', data: [entry1]})
  const request = {
    method: 'POST',
    type: 'entry',
    path: '/entries',
    body: {data: {id: 'ent1', type: 'entry'}}
  }
  const expectedBody = {
    data: {
      id: 'ent1',
