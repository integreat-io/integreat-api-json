...
test('should return response', async (t) => {
  ...
  const ret = createResponse(response, request)
...
test('should return response with one item', async (t) => {
  const response = {status: 'ok', data: entry1}
  const request = {method: 'GET', type: 'entry', id: 'ent1', path: '/entries/ent1'}
  const expectedBody = {
    data: {
      id: 'ent1',
      type: 'entry',
...
test('should return response without relationships', async (t) => {
  ...
  const request = {method: 'GET', type: 'user', id: 'johnf', path: '/users/johnf'}
  const expectedBody = {
    data: {
      id: 'johnf',
      type: 'user',
...
test('should return 404 when not found', async (t) => {
  ...
  const request = {method: 'GET', type: 'entry', id: 'ent0', path: '/entries/ent0'}
