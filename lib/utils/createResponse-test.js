...
test('should return response with paging', async (t) => {
  const response = {
    status: 'ok',
    data: [entry1],
    paging: { next: { type: 'entry', pageSize: 20, pageAfter: 'ent1' } }
  }
  const request = {
    method: 'GET',
    type: 'entry',
    params: { pageSize: 20 },
    path: '/entries/ent1?page=20'
  }
  const action = { type: 'GET', payload: { type: 'entry', id: 'ent1', params: { pageSize: 20 } } }
  const expectedLinks = {
    first: null,
    last: null,
    prev: null,
    next: '/entries/ent1?page=eyJ0eXBlIjoiZW50cnkiLCJwYWdlU2l6ZSI6MjAsInBhZ2VBZnRlciI6ImVudDEifQ%3D%3D'
  }
  ...
  t.truthy(ret)
  t.is(ret.statusCode, 200)
  t.deepEqual(ret.body.links, expectedLinks)
})
