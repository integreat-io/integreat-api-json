...
const setBody = (response, data, paging, request, action, { baseUri } = {}) => {
  ...
  if (paging) {
    const next = createPagePath(paging, request, baseUri)
    response.body.links = {
      first: null,
      last: null,
      prev: null,
 ...
 */
function createResponse ({ status, data, paging, error }, request, action, options) {
  ...
  if (statusCode !== 204 && (status === 'ok' || status === 'noaction')) {
    setBody(response, data, paging, request, action, options)
  }
